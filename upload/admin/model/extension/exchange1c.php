...
class ModelExtensionExchange1c extends Model {
	...
	private function prepareQueryProduct($data, $mode = 'set') {
		...
		if (isset($data['unit_id']) && isset($this->TAB_FIELDS['product']['unit_id'])) {
	 		$sql[] = $mode == 'set'		? "`unit_id` = '" .				(int)$data['unit_id'] . "'"						: "`unit_id`";
		}
		if (isset($data['unit']) && isset($this->TAB_FIELDS['product']['unit'])) {
	 		$sql[] = $mode == 'set'		? "`unit` = '" .				$data['unit'] . "'"						: "`unit`";
		}
...
//		}
		...
		foreach ($xml->ЗначениеРеквизита as $requisite) {
			...
			switch ($name){
				...
				case 'Полное наименование':
					...
					// Полное наименование в первую строку описания
					$data['description'] = $data['full_name'] . "<br /><hr><br />" . $data['description'];   
				break;
				case 'Производитель':
				...
				case 'ЕдиницаИзмерения':
					$data['unit'] = (string)$value;
					$this->log("> Реквизит: " . $name . " = " . $data['unit'], 2);
				break;
	...
	private function parseProducts($xml) {
		...
		foreach ($xml->Товар as $num => $product) {
			...
			// НАИМЕНОВАНИЕ
			if ($product->Наименование && $this->config->get('exchange1c_product_name') == "name") {
				$data['name'] = htmlspecialchars(trim((string)$product->Наименование));
			...
			} elseif ($product->ПолноеНаименование && $this->config->get('exchange1c_product_name') == "fullname") {
			...
			} elseif ($this->config->get('exchange1c_product_name') == "manually") {
				// Название поля наименования
				$field_name = $this->config->get('exchange1c_product_name');
			...
			if ($product->ХарактеристикиТовара == 1) {
...
//		$requisites['СкладДляПодстановкиВЗаказы'] 	= 'Склад основной';
	...
	private function setCustomer($order) {
		...
		$customer = array(
			...
			'Телефон'				=> array('Представление' => $order['telephone']),
			'Email'					=> array('Представление' => $order['email'])
		);
		
		$address = $this->setCustomerAddress($order);
		if ($address) {
			$customer['АдресРегистрации'] = $address;
		}
...
//			$this->log("Обновлено в документе: Итого",2);
	...
	private function parseDocumentCustomer($xml, &$doc) {
		...
		if ($xml->Контрагент->ОфициальноеНаименование && $xml->Контрагент->ИНН && isset($this->TAB_FIELDS['customer']['company_inn'])) {
			$company['name'] = trim((string)$xml->Контрагент->ОфициальноеНаименование);
			$company['inn'] = trim((string)$xml->Контрагент->ИНН);
			$company['kpp'] = trim((string)$xml->Контрагент->КПП);
			$company['type'] = (strlen($company['inn']) == 12) ? 3 : 2;
			$this->log($company);

			// Поиск по организации по ИНН
			$this->log("Поиск организации по ИНН: " . $company['inn']);
			$query = $this->query(
				"SELECT customer_id 
				FROM " . DB_PREFIX . "customer 
				WHERE company_inn = '" . $this->db->escape($company['inn']) . "'"
			);
			if ($query->num_rows) {
				$doc['payment_company'] = $company['name'];
				$doc['shipping_company'] = $company['name'];
			...
			if (!$doc['customer_id']) {
				...
				$doc['company_inn'] = $company['inn'];
				$doc['company_kpp'] = $company['kpp'];
			}
		...
		// Если не организация или не найдена по ИНН
		if (empty($doc['company'])) { 
			...
			// Поиск физического лица в базе (если регистрировался)
			...
			if (!$doc['customer_id']) {
				...
				$this->log("Покупатель не был зарегистрирован на сайте");
			}
	...
	public function defineTableFields() {
		...
		$tables = array(
			...
			'product'					=> array('noindex'=>1,'unit_id'=>0,'unit'=>0),
